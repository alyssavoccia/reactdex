...
function MoveCard({ move }) {
  ...
  return (
    <Link className='relative overflow-hidden shadow-md hover:shadow-lg' to={`/moves/${move.name}`} key={move.name}>
      <div className="flex flex-col bg-white items-center px-4 py-4 rounded-md w-52">
        ...
        <p className="mt-2 font-bold text-slate-500">{moveCapitalName()}</p>
